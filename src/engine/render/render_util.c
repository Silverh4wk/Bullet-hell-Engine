...
renderShaderCreate(const char *path_vert, const char *path_frag) {
    ...
    uint32 shader_vertex = glCreateShader(GL_VERTEX_SHADER);
	...
	uint32 shader_fragment = glCreateShader(GL_FRAGMENT_SHADER);
    ...
    if (!success) {
        ...
        ERROR_EXIT("Error linking the program: %s", log);
    }
