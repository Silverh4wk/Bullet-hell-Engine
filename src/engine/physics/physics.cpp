...
{
    return (Body*) array_list_get(state.body_list,index);
}

int testAABBAABB(AABB a, AABB b){
    
// Exit with no intersection if separated along an axis
    if( abs((int)a.position[0] - (int)b.position[0]) > (a.radius[0] + b.radius[0])) return 0;
    if( abs((int)a.position[1] - (int)b.position[1]) > (a.radius[1] + b.radius[1])) return 0;
...
Body* physicsGetBody(size_t i) { return (Body*) array_list_get(state.body_list, i); }
...
void physicsToggleAllHitBoxes(void)
{
    ...
    {
	Body* body = (Body*) array_list_get(state.body_list, i);
